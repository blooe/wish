	...
	$('#send-btn').click(function(){
		...
		$.post(handleUrl,{username:username.val(), content:content.val()},function (data){
			if(data.status){
				...
				str +="<dt><span class='username'>"+data.username+"</span><span class='num'>";
				str +=data.id+"</span></dt><dd class='content'>";
				str +=data.content+"</dd><dd class='bottom'><span class='time'>";
				str +=data.time+"</span><a href='' class='close'></a></dd></dl>";
				$('#main').append(str);
				$('#close').click();
			}else {
				alert("发布失败");
